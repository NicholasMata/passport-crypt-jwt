...
const header = require('./parse_header');
 ...
 *          isEncrypted: If true then will decrypt the token with the secretOrKey.
...
function JwtStrategy(options, verify) {
  ...
  if(options.isEncrypted != null) {
    this._verifOpts.isEncrypted = options.isEncrypted;
  }
};
