...
module.exports = function (payload, secretOrPrivateKey, options, callback) {
  ...
  if(typeof options.shouldEncrypt === 'undefined') {
    options.shouldEncrypt = false
  }
  ...
  if (typeof callback === 'function') {
    ...
    .once('done', function (signature) {
      ...
      if(options.shouldEncrypt) {
        token = crypt.encrypt(token, secretOrPrivateKey);
      }
      callback(null, signature);
    });
  } else {
    var token = jws.sign({header: header, payload: payload, secret: secretOrPrivateKey, encoding: encoding});
    if(options.shouldEncrypt) {
      token = crypt.encrypt(token, secretOrPrivateKey);
    }
    return token
